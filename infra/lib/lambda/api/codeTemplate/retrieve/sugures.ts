...
export class SuguresRetriever extends BaseRetriever {
  ...
  async getRelevantDocuments(query: string): Promise<Document[]> {
    const suguresDocuments = await getSugures(query, this.subscriptionId, this.conversationId);
 ...
 * @returns 
 */
export const getSugures = async (query: string, subscriptionId: string, conversationId: string): Promise<Document[]> => {
  try {
    ...
    if (messages[0].type == SuguresMessageType.text
      ...
      suguresWasHelpfulPostback(wasHelpfulPostbackKeys, wasNotHelpfulPostbackKeys, subscriptionId, conversationId, sessionId)
