...
export const listHandler = async (req: Request, res: Response) => {
  ...
  let retErrorCode: ErrorCode = ErrorCode.A10099;
  ...
  try {
    ...
    const conversationHistoryList = await selectConversationHistory(dbClient, body, limit);

    // 終了
    res.status(200).json({ conversationHistoryList: conversationHistoryList.rows });
  } catch (err) {
    // エラーログの出力
    const errorMessage = ({
      subscriptionId: subscriptionId,
